...
import { propertyService, uploadService } from "@/lib/services/property.service";
...
export function PropertiesDashboard({
  ...
  const handleStartCreate = () => {
    ...
    setFormOwners(initialOwners.length > 0 ? [{ ownerId: initialOwners[0].id, share: 100 }] : []);
    setFormImages([]);
    setActiveTab("info");
    setIsEditing(true);
  };
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.error("Erreur inattendue lors de l'envoi du fichier.", { id: toastId });
    } finally {
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (formOwners.length > 0) {
      const totalShare = formOwners.reduce((acc, o) => acc + Number(o.share), 0);
      if (Math.abs(totalShare - 100) > 0.01) {
        toast.error(`La somme des parts d'indivision doit être de 100% (actuelle: ${totalShare}%)`);
        setActiveTab("owners");
        return;
      }
    }
    ...
    startTransition(async () => {
      const payload = {
        ...
        owners: formOwners.map((o) => ({ ownerId: o.ownerId, share: Number(o.share) })),
      ...
      if (selectedProperty) {
        const res = await propertyService.updateProperty(selectedProperty.id, payload);
  ...
  const presetPhotos = [
    { label: "Façade Villa", url: "https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?auto=format&fit=crop&w=800&q=80" },
    { label: "Salon Moderne", url: "https://images.unsplash.com/photo-1600210492486-724fe5c67fb0?auto=format&fit=crop&w=800&q=80" },
    { label: "Cuisine Équipée", url: "https://images.unsplash.com/photo-1556911220-e15b29be8c8f?auto=format&fit=crop&w=800&q=80" },
    { label: "Chambre Principale", url: "https://images.unsplash.com/photo-1595526114035-0d45ed16cfbf?auto=format&fit=crop&w=800&q=80" },
  ];
              ...
              <TableBody>
                {currentBatch.length === 0 ? (
                  ...
                  currentBatch.map((prop) => {
                    ...
                    const coverImg = prop.images.find((i) => i.isCover)?.url || prop.images[0]?.url;
                      ...
                      >
                        ...
                        <TableCell>
                          {prop.status === "AVAILABLE" && (
                            <Badge variant="default" dot>Disponible</Badge>
                          )}
                          {prop.status === "RENTED" && (
                            <Badge variant="destructive" dot>Loué</Badge>
                          )}
                          {prop.status === "UNDER_MAINTENANCE" && (
                            <Badge variant="surface" dot>En travaux</Badge>
                          )}
              ...
              <div className="flex border-b border-outline-variant/60 pt-2 gap-4 text-sm font-semibold">
                ...
                >
                  ...
                  <span className="px-1.5 py-0.5 text-xs bg-surface-container-high rounded-full">
                    {isEditing ? formOwners.length : (selectedProperty?.owners?.length || 0)}
                  </span>
                </button>
                <button
                  type="button"
                ...
                >
                  Photos
                  <span className="px-1.5 py-0.5 text-xs bg-surface-container-high rounded-full">
                    {isEditing ? formImages.length : (selectedProperty?.images?.length || 0)}
                  </span>
                </button>
              </div>
            </CardHeader>
              ...
              {isEditing ? (
                <form onSubmit={handleSave} className="flex flex-col gap-md">
                      ...
                      <div className="grid grid-cols-2 gap-sm">
                        ...
                        <div className="flex flex-col gap-xs">
                          ...
                          <Input
                            ...
                            onChange={(e) =>
                              setFormData({ ...formData, baseRent: e.target.value === "" ? "" : Number(e.target.value) })
                            }
                          />
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-sm">
                        <div className="flex flex-col gap-xs">
                          ...
                          <Input
                            ...
                            value={formData.area}
                            onChange={(e) =>
                              setFormData({ ...formData, area: e.target.value === "" ? "" : Number(e.target.value) })
                            }
                          />
                        </div>
                        <div className="flex flex-col gap-xs">
                          ...
                          <Input
                            ...
                            onChange={(e) =>
                              setFormData({ ...formData, roomsCount: e.target.value === "" ? "" : Number(e.target.value) })
                            }
                          />
                        </div>
                      </div>
                  ...
                  {activeTab === "owners" && (
                    <div className="flex flex-col gap-md animate-fade-in">
                      <div className="flex items-center justify-between">
                        <h4 className="text-body-lg font-bold text-on-surface">Répartition de propriété</h4>
                        <Button type="button" variant="outline" size="sm" onClick={handleAddOwnerShare}>
                          <span className="material-symbols-outlined text-sm mr-1 select-none" data-icon="add">add</span>
                          Ajouter Bailleur
                        </Button>
                      </div>
                      ...
                      {formOwners.length === 0 ? (
                        <p className="text-body-sm text-on-surface-variant bg-surface-container p-4 rounded text-center">
                          Aucun propriétaire sélectionné. La propriété sera enregistrée sans rattachement.
                        </p>
                      ) : (
                        <div className="flex flex-col gap-2">
                          {formOwners.map((o, idx) => (
                            <div key={idx} className="flex items-center gap-2 bg-surface-container-lowest p-2 rounded border border-outline-variant/60">
                              <div className="flex-1">
                                <Select
                                  value={o.ownerId}
                                  onChange={(e) => handleUpdateOwnerShare(idx, e.target.value, o.share)}
                                  options={initialOwners.map((own) => ({
                                    label: `${own.firstName || ""} ${own.lastName || ""} ${own.companyName ? `(${own.companyName})` : ""}`.trim() || own.id,
                                    value: own.id,
                                  }))}
                              ...
                              <div className="w-24 flex items-center gap-1">
                                <Input
                                  ...
                                  placeholder="%"
                                  value={o.share}
                                  onChange={(e) => handleUpdateOwnerShare(idx, o.ownerId, Number(e.target.value))}
                              ...
                              {formOwners.length > 1 && (
                                ...
                                >
                                  <span className="material-symbols-outlined select-none" data-icon="delete">delete</span>
                                </Button>
                              )}
                            </div>
                          ))}
                          ...
                          <div className="flex justify-between items-center p-3 mt-2 bg-surface-container rounded-lg border border-outline-variant">
                            <span className="font-semibold text-body-sm">Total des parts d&apos;indivision :</span>
                            <span
                              className={`font-mono font-bold text-lg ${
                                formOwners.reduce((acc, o) => acc + Number(o.share), 0) === 100
                                  ? "text-primary"
                                  : "text-error"
                              }`}
                            >
                              {formOwners.reduce((acc, o) => acc + Number(o.share), 0)}%
                            </span>
                          </div>
                        </div>
                      )}
                  ...
                  {activeTab === "media" && (
                    <div className="flex flex-col gap-md animate-fade-in">
                      <div className="flex flex-col gap-2">
                        <h4 className="text-body-lg font-bold text-on-surface">Galerie de Photos</h4>
                        <p className="text-body-sm text-on-surface-variant">
                          Ajoutez des photos de présentation. La première photo cochée en couverture sera affichée sur les catalogues.
                        </p>
                      </div>
                      ...
                      <div className="flex flex-col items-center justify-center gap-2 bg-surface-container-lowest p-6 rounded-xl border-2 border-dashed border-primary/50 text-center hover:bg-primary/5 transition-colors cursor-pointer relative shadow-xs">
                        ...
                        />
                        <span className="material-symbols-outlined text-4xl text-primary mx-auto mb-1 select-none" data-icon="cloud_upload">
                          cloud_upload
                        </span>
                        <span className="font-bold text-body-md text-on-surface">
                          {isUploading ? "Envoi en cours..." : "Cliquez ou glissez une image"}
                        </span>
                        <span className="text-xs text-on-surface-variant">Formats acceptés : JPG, PNG, WEBP (Max 5Mo)</span>
                      </div>

                      <div className="flex flex-col gap-2 bg-surface-container-lowest p-3 rounded-xl border border-outline-variant mt-1">
                        <span className="text-xs font-bold uppercase text-on-surface-variant">Ajouter par URL externe</span>
                        <div className="flex gap-2">
                          ...
                          />
                          <Button type="button" onClick={handleAddCustomImage} size="sm">
                            Ajouter
                          </Button>
                        </div>
                        <Input
                      ...
                      <div className="flex flex-col gap-1.5">
                        <span className="text-xs font-bold uppercase text-on-surface-variant">Ou utiliser des photos de démonstration :</span>
                        <div className="grid grid-cols-2 gap-2">
                          {presetPhotos.map((preset, idx) => (
                            ...
                            >
                              <img src={preset.url} alt="" className="w-10 h-10 rounded object-cover" />
                      ...
                      <div className="flex flex-col gap-3 mt-2">
                        <span className="text-label-caps uppercase text-on-surface-variant font-semibold">Photos enregistrées ({formImages.length})</span>
                        {formImages.length === 0 ? (
                          <p className="text-body-sm text-on-surface-variant bg-surface-container p-4 rounded text-center">
                            ...
                            {formImages.map((img, idx) => (
                              <div
                                ...
                                className={`relative group rounded-xl overflow-hidden border-2 transition-all ${
                                  img.isCover ? "border-primary shadow-md" : "border-outline-variant/60"
                                }`}
                              >
                                  ...
                                  <div className="flex justify-between items-center">
                                    <button
                                      ...
                                      className={`px-2 py-0.5 rounded-full text-xs font-bold transition-all ${
                                        img.isCover ? "bg-primary text-on-primary" : "bg-black/60 text-white hover:bg-black"
                                      }`}
                                    >
                                      {img.isCover ? "★ Couverture" : "Définir couverture"}
                                    </button>
                                    <button
                                      type="button"
                                    ...
                                    >
                                      <span className="material-symbols-outlined text-sm select-none" data-icon="close">close</span>
                                    </button>
                                  </div>
                  ...
                  <Button type="submit" disabled={isPending || isUploading} className="w-full mt-lg">
                ...
                <div className="flex flex-col gap-md">
                    ...
                    <div className="flex flex-col gap-md animate-fade-in">
                      ...
                      <div className="flex flex-col gap-sm text-left">
                        <div className="flex justify-between items-center bg-surface-container-lowest p-2.5 rounded-lg border border-outline-variant/40">
                          <span className="text-label-caps uppercase text-on-surface-variant">Loyer de base</span>
                        ...
                        <div className="flex justify-between items-center bg-surface-container-lowest p-2.5 rounded-lg border border-outline-variant/40">
                          ...
                          <span>
                            {selectedProperty.status === "AVAILABLE" && <Badge variant="default">Disponible</Badge>}
                            {selectedProperty.status === "RENTED" && <Badge variant="destructive">Loué</Badge>}
                            {selectedProperty.status === "UNDER_MAINTENANCE" && <Badge variant="surface">En travaux</Badge>}
                            {selectedProperty.status === "UNAVAILABLE" && <Badge variant="surface">Indisponible</Badge>}
                          </span>
                        </div>

                        <div className="flex justify-between items-center bg-surface-container-lowest p-2.5 rounded-lg border border-outline-variant/40">
                          <span className="text-label-caps uppercase text-on-surface-variant">Type de bien</span>
                          <span className="text-body-sm text-on-surface font-semibold">{selectedProperty.propertyType}</span>
                        </div>

                        <div className="flex justify-between items-center bg-surface-container-lowest p-2.5 rounded-lg border border-outline-variant/40">
                          <span className="text-label-caps uppercase text-on-surface-variant">Surface / Pièces</span>
                          <span className="text-body-sm text-on-surface font-semibold">
                            {selectedProperty.area ? `${selectedProperty.area} m²` : "N/D"} / {selectedProperty.roomsCount || "N/D"}
                          </span>
                        </div>

                        {selectedProperty.description && (
                          <div className="bg-surface-container-lowest p-3 rounded-lg border border-outline-variant/40 mt-1">
                            <span className="text-label-caps uppercase text-on-surface-variant block mb-1">Description</span>
                            <p className="text-body-sm leading-relaxed text-on-surface-variant">{selectedProperty.description}</p>
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  {activeTab === "owners" && (
                    <div className="flex flex-col gap-3 animate-fade-in">
                      <h4 className="text-body-lg font-bold text-on-surface">Bailleurs en Indivision</h4>
                      {selectedProperty.owners && selectedProperty.owners.length > 0 ? (
                        <div className="flex flex-col gap-2.5">
                          {selectedProperty.owners.map((o) => (
                            <div key={o.id} className="flex flex-col bg-surface-container-lowest p-3 rounded-xl border border-outline-variant shadow-xs">
                              <div className="flex justify-between items-center">
                                <span className="font-bold text-body-md text-on-surface">{o.fullName}</span>
                  ...
                  {activeTab === "media" && (
                    <div className="flex flex-col gap-3 animate-fade-in">
                      <h4 className="text-body-lg font-bold text-on-surface">Photos & Médias</h4>
                          ...
                          {selectedProperty.images.map((img) => (
                            <div
                              ...
                              className={`relative group rounded-xl overflow-hidden border ${
                                img.isCover ? "border-primary border-2 shadow-md" : "border-outline-variant"
                              }`}
                            >
                  ...
                  <Button onClick={() => handleStartEdit(selectedProperty)} className="w-full mt-sm">
                ...
                <div className="py-xl flex flex-col items-center text-on-surface-variant text-center">
                  <span className="material-symbols-outlined text-4xl mb-sm opacity-60" data-icon="apartment">
                    apartment
                  </span>
                  <p className="text-body-md">
                    Sélectionnez une propriété pour voir ses détails ou ajoutez-en une nouvelle.
                  </p>
                </div>
              )}
            </CardContent>
