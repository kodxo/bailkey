...
export function TableSkeleton(): React.JSX.Element {
  return (
    <Table wrapperClassName="max-h-[calc(100vh-250px)] rounded-xl border border-outline-variant/60 shadow-xs relative bg-surface-container-lowest">
      <TableHeader>
        <TableRow>
        ...
        {Array.from({ length: SKELETON_ROWS }).map((_, i) => (
          <TableRow key={i} className="animate-pulse">
            ...
            <TableCell className="text-right">
              <div className="h-8 w-16 bg-outline-variant/20 rounded ml-auto" />
            </TableCell>
          </TableRow>
        ))}
      </TableBody>
